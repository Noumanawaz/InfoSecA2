...
def main():
	...
	with socket.socket() as s:
		...
		seq=0
		ct_good = b64e(aesmod.encrypt_cbc(K,iv,b"hello"))
		ts = now_ms()
		sig = b64e(rsamod.rsa_sign_sha256(priv, dict_to_bytes({"seq":seq,"ts":ts,"ct":ct_good})))
		ct_bad = b64e(aesmod.encrypt_cbc(K,iv,b"tamper"))
		send_json(s, EncryptedMessage(type="msg", seq=seq, ts=ts, ct=ct_bad, sig=sig).dict())
		print(recv_json(s))
