...
def main():
	...
	with socket.socket() as s:
		...
		# Session DH
		K = do_dh_exchange(s)
		# Send tampered message: sign over one ct but send another
		seq=0
		ivm = rand_bytes(16)
		ct_good = b64e(ivm + aesmod.encrypt_cbc(K,ivm,b"hello"))
		ts = now_ms()
		sig = b64e(rsamod.rsa_sign_sha256(priv, dict_to_bytes({"seq":seq,"ts":ts,"ct":ct_good})))
		ivm2 = rand_bytes(16)
		ct_bad = b64e(ivm2 + aesmod.encrypt_cbc(K,ivm2,b"tamper"))
		send_json(s, EncryptedMessage(type="msg", seq=seq, ts=ts, ct=ct_bad, sig=sig).model_dump())
		print(recv_json(s))
